...
import { Upload, TrendingUp, TrendingDown, DollarSign, Users } from "lucide-react";
...
const DashboardCharts = () => {
  ...
  const marketingBreakdown = useMemo(() => {
    const marketingFields = ["marketing_text", "marketing_social_media", "marketing_print", "marketing_radio", "marketing_tv", "marketing_website"];
    const result: Record<string, number> = {};
    filteredData.filter(d => marketingFields.includes(d.field_name)).forEach(d => {
      const label = d.field_label;
      result[label] = (result[label] || 0) + formatCurrency(d.field_value);
    });
    return Object.entries(result).filter(([, value]) => value > 0).map(([name, value]) => ({ name, value }));
  }, [filteredData]);
  ...
  const monthlyComparisons = useMemo(() => {
    ...
    filteredData.forEach(d => {
      ...
      if (!groupedByPeriod[period]) groupedByPeriod[period] = { psc: 0, sales: 0, defaults: 0 };
      if (d.field_name === "psc_collected") groupedByPeriod[period].psc = formatCurrency(d.field_value);
      if (d.field_name === "retail_sales") groupedByPeriod[period].sales = formatCurrency(d.field_value);
      if (d.field_name === "dollar_pawns_defaulted") groupedByPeriod[period].defaults = formatCurrency(d.field_value);
    });
    return Object.entries(groupedByPeriod).map(([period, data]) => ({ period, ...data }));
  }, [filteredData]);
  ...
  const summaryStats = useMemo(() => {
    ...
    const latestData = filteredData.filter(d => d.year === latestMonth.year && d.month === latestMonth.month);
    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Pawn Balance</CardTitle>
              <DollarSign className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">${summaryStats.pawnBalance.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">{summaryStats.period}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">PSC Collected</CardTitle>
              <TrendingUp className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">${summaryStats.pscCollected.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">{summaryStats.period}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Retail Sales</CardTitle>
              <TrendingDown className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">${summaryStats.retailSales.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">{summaryStats.period}</p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Pawn Customers</CardTitle>
              <Users className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{summaryStats.pawnCustomers.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">{summaryStats.period}</p>
            </CardContent>
          </Card>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {pawnBalanceTrend.length > 0 && (
          <Card>
            <CardHeader><CardTitle>Pawn Balance Trend</CardTitle></CardHeader>
            <CardContent>
        ...
        {marketingBreakdown.length > 0 && (
          <Card>
            <CardHeader><CardTitle>Marketing Spend Breakdown</CardTitle></CardHeader>
            <CardContent>
              <ChartContainer config={chartConfig} className="h-[350px] w-full">
                <PieChart margin={{ top: 20, right: 80, bottom: 20, left: 80 }}>
                  <Pie
                    ...
                    label={({ cx, cy, midAngle, outerRadius, name, percent }) => {
                      ...
                      return (
                        <text x={x} y={y} textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" className="fill-foreground text-xs">
